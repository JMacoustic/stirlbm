...
void main_setup() { // without decay
	...
	std::uniform_int_distribution<int> dist3(MAGNIFY_RANGE);

	float rpm = dist2(gen);
	int magnify = dist3(gen);
	selected.dynamic_viscosity *= 1000; // Pa*s to cP
	...
	const float si_nu = selected.kinematic_viscosity * magnify; // water: 0.000001
	...
	const ulong lbm_dt = STEPS; // Simulation time resolution
