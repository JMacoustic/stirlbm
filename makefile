MAKEFLAGS = -j$(nproc)
CC = g++
CFLAGS =  -lstdc++fs -std=c++17 -pthread -O -Wno-comment
...
temp/setup.o: src/setup.cpp src/defines.hpp src/graphics.hpp src/info.hpp src/lbm.hpp src/lodepng.hpp src/opencl.hpp src/setup.hpp src/shapes.hpp src/units.hpp src/utilities.hpp src/fluiddata.hpp make.sh
	@mkdir -p temp
	$(CC) -c src/setup.cpp -o temp/setup.o $(CFLAGS) $(LDFLAGS_OPENCL)

temp/fluiddata.o: src/fluiddata.cpp src/fluiddata.hpp src/json.hpp
	@mkdir -p temp
	$(CC) -c src/fluiddata.cpp -o temp/fluiddata.o $(CFLAGS)
